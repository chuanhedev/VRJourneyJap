...
void Start()
    ...
    void Update () 
	{
...
#endif
		{
			...
			{
				float h = 40.0f * -Input.GetAxis("Mouse X") * Time.unscaledDeltaTime;
				float v = 40.0f * Input.GetAxis("Mouse Y") * Time.unscaledDeltaTime;
				h = Mathf.Clamp(h, -0.1f, 0.1f);
				v = Mathf.Clamp(v, -0.1f, 0.1f);
				_spinX += h;
				_spinY += v;
			}
			if (!Mathf.Approximately(_spinX, 0f) || !Mathf.Approximately(_spinY, 0f))
			{
                ...
                _spinX = Mathf.MoveTowards(_spinX, 0f, 5f * Time.unscaledDeltaTime);
				_spinY = Mathf.MoveTowards(_spinY, 0f, 5f * Time.unscaledDeltaTime);
			}
		}
	}
#endif
