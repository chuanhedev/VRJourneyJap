...
public static class ZBMFontReader
{
	/// <summary>
	...
	static public void Load (ZBMFont font, string name, byte[] bytes)
	{
		font.Clear();

		if (bytes != null)
		{
			ZByteReader reader = new ZByteReader(bytes);
			...
			while (reader.canRead)
			{
				...
				if (split[0] == "char")
				{
					...
					if (len > 8)
					{
						...
						ZBMGlyph glyph = font.GetGlyph(id, true);
				...
				else if (split[0] == "kerning")
				{
					...
					if (len > 3)
					{
						...
						ZBMGlyph glyph = font.GetGlyph(second, true);
