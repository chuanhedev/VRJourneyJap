...
public class ZBMFont
{
	...
	[HideInInspector][SerializeField] List<ZBMGlyph> mSaved = new List<ZBMGlyph>();

	// Actual glyphs that we'll be working with are stored in a dictionary, making the lookup faster
	private Dictionary<int, ZBMGlyph> mDict = new Dictionary<int, ZBMGlyph>();
    ...
    /// </summary>
    /// <returns></returns>
    public Dictionary<int, ZBMGlyph> GetGlyphs() {
        return mDict;
    }
	...
	public ZBMGlyph GetGlyph (int index, bool createIfMissing)
	{
	...
	public ZBMGlyph GetGlyph (int index) { return GetGlyph(index, false); }
