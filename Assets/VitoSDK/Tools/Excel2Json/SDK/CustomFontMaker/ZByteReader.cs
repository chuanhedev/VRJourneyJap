...
public class ZByteReader
{
	byte[] mBuffer;
	int mOffset = 0;

	public ZByteReader (byte[] bytes) { mBuffer = bytes; }
	public ZByteReader (TextAsset asset) { mBuffer = asset.bytes; }
	...
	static ZBetterList<string> mTemp = new ZBetterList<string>();

	/// <summary>
	/// Read a single line of Comma-Separated Values from the file.
	/// </summary>

	public ZBetterList<string> ReadCSV ()
	{
		mTemp.Clear();
